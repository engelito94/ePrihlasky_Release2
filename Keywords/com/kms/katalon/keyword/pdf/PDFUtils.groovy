...
public class PDFUtils {
	...
	static String getPDFText(String file, int startPage, int endPage) throws IOException{
		...
		KeywordUtil.logInfo("súbor : " + file);
		KeywordUtil.logInfo("prvá strana : " + startPage);
		KeywordUtil.logInfo("posledná strana : " + endPage);
		...
		KeywordUtil.logInfo("PDF Text pred odstránením prázdnych znakov : " + txt);
		if(PDFUtils.bTrimWhiteSpace){
			txt = txt.trim().replaceAll("\\s+", " ").trim();
			KeywordUtil.logInfo("PDF Text po odstránení prázdnych znakov : " + txt);
		}
	...
	static boolean comparepdfFilesWithTextMode(String file1, String file2, int startPage, int endPage, String[] excludePattern) throws IOException{
		...
		if(!result){
			KeywordUtil.markFailed("PDF-ká sa nezhodujú")
		}
		else {
			KeywordUtil.markPassed("PDF-ká sa zhodujú");
		}
		return result;
	}
}
